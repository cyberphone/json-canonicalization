...
//
// Modifications for ES6/JCS compliance by Anders Rundgren
//
...
/**
 * An implementation of Ryu for serializing IEEE-754 double precision
 * values for JSON as specified by ES6
 */
public final class NumberToJSON {
  ...
  public static String serializeNumber(double value) throws IOException {
    ...
    //
    // We do some extra work here in order to follow ES6 semantics. In particular,
    // that requires printing in scientific format if and only if the exponent is
    // between -6 and 21, and it requires suppressing .0 
    //
    // Above, we moved the decimal dot all the way to the right, so now we need to
    // count digits to figure out the correct exponent for scientific notation.
    final int vplength = decimalLength(dp);
    int exp = e10 + vplength - 1;

    // ES6/JCS semantics requires using scientific notation if and only if outside this range.
